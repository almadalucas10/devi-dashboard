...
// Lead times em dias (placeholder — PCP deve definir por família)
const LEAD_TIMES = {
  kombucha: 10,  // FX*
  cha: 7,        // CH*
  refri: 5,      // RF*, RTM*
...
export async function buscarVendasPorSku(env) {
  ...
  do {
    ...
    for (const pv of pedidos) {
      ...
      // Filtra etapas: só conta o que já passou de "A faturar" (etapa > 50)
      const etapa = cab.etapa || "0";
      const etapaNum = parseInt(etapa, 10);
      if (etapaNum > 0 && etapaNum < 50) continue; // ainda não saiu
  ...
  console.log(`✅ Vendas: ${Object.values(saida).reduce((a,b)=>a+b,0)} un em ${DIAS_JANELA}d`);
  return saida;
}
...
export function calcularCobertura(estoque, saidaPorSku) {
  ...
  for (const item of estoque) {
    ...
    if (diaria < LIMIAR_GIRO / DIAS_JANELA || saidaTotal === 0) {
      semGiro.push(sku);
      todos.push({
        codigo: sku,
        nome: item.descricao || NOME_CURTO[sku] || sku,
        saldo: item.saldo,
  ...
  // Ordena por cobertura (menor primeiro), sem_giro no final
  todos.sort((a, b) => {
